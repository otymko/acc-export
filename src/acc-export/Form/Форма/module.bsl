...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	ЗаписьЖурналаРегистрации("Выгрузка в сонар", УровеньЖурналаРегистрации.Информация,,, НСтр("ru='Запуск выгрузки в сонар'"));
	...
	Если ТипЗнч(Конфигурация) <> Тип("СправочникСсылка.Конфигурации") Тогда		
		...
		ЗаписьЖурналаРегистрации("Выгрузка в сонар", УровеньЖурналаРегистрации.Ошибка,,, НСтр("ru='Неправильный реквизит обработки ""Конфигурация"".'"));
	...
	Если Не РежимОтладки Тогда
		...
		Если ПараметрыЗаполненыКорректно Тогда			
			ВыгрузитьОшибки();                			
		Иначе                                 			
			ЗаписьЖурналаРегистрации("Выгрузка в сонар", УровеньЖурналаРегистрации.Ошибка,,, НСтр("ru='Параметры заданы некорректно'")); 			
		КонецЕсли;
...
Функция ПроверитьЗаполнениеПараметровОбработки()
	...
	Если Конфигурация.Пустая() Тогда
		...
		ЗаписьЖурналаРегистрации("Выгрузка в сонар", УровеньЖурналаРегистрации.Ошибка,,, НСтр("ru='Конфигурация не найдена(acc.projectKey)'"));
	КонецЕсли;
	
	Если Не КаталогСуществует(КаталогПроекта) Тогда
		Результат = Ложь;
		
		ЗаписьЖурналаРегистрации("Выгрузка в сонар", УровеньЖурналаРегистрации.Ошибка,,, НСтр("ru='Каталог проекта не найден(acc.catalog)'"));
	КонецЕсли;
	
	Если Не КаталогСуществует(КаталогИсходныхКодов) Тогда
		Результат = Ложь;
		
		ЗаписьЖурналаРегистрации("Выгрузка в сонар", УровеньЖурналаРегистрации.Ошибка,,, НСтр("ru='Каталог исходных кодов не найден(acc.sources)'"));
	КонецЕсли;
	
	Если ПустаяСтрока(ФорматЭкспорта) Тогда
		Результат = Ложь;
		
		ЗаписьЖурналаРегистрации("Выгрузка в сонар", УровеньЖурналаРегистрации.Ошибка,,, НСтр("ru='Формат экспорта не задан(acc.format)'"));
	КонецЕсли;
	
	Если ПустаяСтрока(ФорматПредставленияОшибки) Тогда
		Результат = Ложь;
		
		ЗаписьЖурналаРегистрации("Выгрузка в сонар", УровеньЖурналаРегистрации.Ошибка,,, НСтр("ru='Формат представления ошибки не задан(acc.titleError)'"));
	КонецЕсли;
...
Процедура ПостОбработкаПараметров()
	...
	Если ЗначениеЗаполнено(ИмяПроекта) Тогда
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации("Выгрузка в сонар", УровеньЖурналаРегистрации.Ошибка,,, НСтр("ru='Не найдена конфигурация для проверки'"));
...
Процедура ОбеспечитьАбсолютныйПутьККаталогуИсходныхКодов()
	...
	Лог_Отладка(ОписаниеОшибки);
	ЗаписьЖурналаРегистрации("Выгрузка в сонар", УровеньЖурналаРегистрации.Ошибка,,, ОписаниеОшибки);
...
Функция НайтиКаталогИсходныхКодов()
	...
	Если КаталогСуществует(КаталогДляПроверки) Тогда		
		...
		Лог_Отладка("Каталог исходных файлов по переданному пути: " + КаталогДляПроверки);		
