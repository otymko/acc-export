...
Процедура ОбеспечитьАбсолютныйПутьКФайлуКлассификацииОшибок()
	...
	ЭтоПолныйПуть = СтрНайти(ФайлКлассификацииОшибок, ":") > 0;
	
	Если ЭтоПолныйПуть Тогда
		
		Если ФайлСуществует(ФайлКлассификацииОшибок) Тогда
			
			Лог_Отладка("Файл классификации ошибок по переданному полному пути: " + ФайлКлассификацииОшибок);
	...
	Иначе
		...
		Если Не ФайлСуществует(ФайлКлассификацииОшибок) Тогда
...
Процедура ПостОбработкаВывестиПараметрыВСообщения()
	...
	Лог_Отладка("Выполнять проверку = " + ВыполнятьПроверку);
	Лог_Отладка("Формат экспорта = " + ФорматЭкспорта);
	Лог_Отладка("Файл классификации ошибок = " + ФайлКлассификацииОшибок);
