...
Перем ОтсутствующиеВПроверкеНастройкиОшибок Экспорт;
Перем АдресФайлаКлассификацииВоВременномХранилище Экспорт;
...
Процедура Лог_Информация(ТекстСообщения) Экспорт
	...
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = "[" + ТекущаяДата() + "] acc-export: " + ТекстСообщения;
	Сообщение.Сообщить();
...
Функция ДанныеОшибки(ВыборкаОшибки)
	...
	структОшибки.Вставить("Серьезность", ВыборкаОшибки.Серьезность);
	структОшибки.Вставить("ТипОшибки", ВыборкаОшибки.ТипОшибки);
	структОшибки.Вставить("ЗатрачиваемыеУсилия", ВыборкаОшибки.ЗатрачиваемыеУсилия);
	...
	Иначе
		...
		Лог_Информация("		Текущий объект: " + структОшибки.Объект);
		Лог_Информация("		Тип объекта: " + структОшибки.ТипОбъекта);
		Лог_Информация("		Место обнаружения: " + структОшибки.МестоОбнаружения);
		Лог_Информация("		Текст ошибки: " + структОшибки.ТекстОшибки);
		Лог_Информация("		Уточнение: " + структОшибки.Уточнение);
		Лог_Информация("		Путь к объекту: " + структОшибки.Путь);
...
Функция ПеренаправитьНаМодульУправляемогоПриложения(Знач ОписаниеОшибки)
	...
	Если ОписаниеОшибки.ТипОбъекта = Перечисления.ТипыОбъектов.Роль
		...
		ИЛИ ОписаниеОшибки.ТипОбъекта = Перечисления.ТипыОбъектов.Константа
		ИЛИ ОписаниеОшибки.ТипОбъекта = Перечисления.ТипыОбъектов.Перечисление
		ИЛИ ОписаниеОшибки.ТипОбъекта = Перечисления.ТипыОбъектов.ПланОбмена
		ИЛИ ОписаниеОшибки.ТипОбъекта = Перечисления.ТипыОбъектов.ЖурналДокументов Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция АбсолютныйПутьОбъекта(ТекущийОбъект, ТипОбъекта, МестоОбнаружения, ПутьКОбъекту)
	...
	Если СтрокаПоискаСинонима <> Неопределено Тогда
		...
		Иначе
			Лог_Информация("Неопознаный модуль: " + МестоОбнаружения);
		КонецЕсли;
...
Функция СформироватьТаблицуКлассификации()
	...
	ТаблицаКлассификации.Колонки.Добавить("Серьезность", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(10, ДопустимаяДлина.Переменная)));
	ТаблицаКлассификации.Колонки.Добавить("Тип", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(20, ДопустимаяДлина.Переменная)));
	ТаблицаКлассификации.Колонки.Добавить("ЗатрачиваемыеУсилия", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(3, 0, ДопустимыйЗнак.Неотрицательный)));
	ТаблицаКлассификации.Колонки.Добавить("КодОшибки", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(длинаКода, ДопустимаяДлина.Переменная)));
	ТаблицаКлассификации.Колонки.Добавить("НаименованиеОшибки", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(150, ДопустимаяДлина.Переменная)));
