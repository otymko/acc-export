...
Функция ВыгрузитьОшибкиАПКGenericIssue(РезультатыПроверки, МассивГруппировок)
	...
	Для Каждого Группировка Из МассивГруппировок Цикл
		...
		Для Каждого СтрокаПроверки Из СтрокиПроверки Цикл
			...
			Запись.ЗаписатьЗначение(СтрокаПроверки.ЗатрачиваемыеУсилия);
...
Функция СформироватьТаблицуКлассификации()
	...
	ТаблицаКлассификации.Колонки.Добавить("ЗатрачиваемыеУсилия", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2, 0, ДопустимыйЗнак.Неотрицательный)));
	ТаблицаКлассификации.Колонки.Добавить("КодОшибки", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(13, ДопустимаяДлина.Переменная)));
...
>>>>>>> 4aa9029... Issues 53: Сопоставление серьезности, типов ошибок АПК и SonarQube, дублирование ошибок платформенной проверки
#КонецОбласти