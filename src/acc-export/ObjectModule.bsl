...
Процедура ЭкспортироватьОшибкиАПК() Экспорт
	...
	Если ВыборкаОшибокАПК = Неопределено Тогда
		...
		Лог_Отладка("Нет данных для выгрузки", ВыводитьСообщения);
		Возврат;
	КонецЕсли;
...
Процедура Лог_Отладка(ТекстСообщения, ВыводитьСообщение = Ложь) Экспорт 
	
	Если РежимОтладки
		Или ВыводитьСообщение Тогда		
...
Функция ДанныеОшибки(ВыборкаОшибки)
	...
	ИначеЕсли ПеренаправлятьНекуда(структОшибки) Тогда
		...
		Лог_Отладка("Не удалось обнаружить место замечания. Содержание замечания: " + структОшибки.ТекстОшибки, ВыводитьСообщения);
		
	Иначе
		
		ТекстСообщения = "Не удалось перенаправить:
		|		Текущий объект: " + структОшибки.Объект + "
		|		Тип объекта: " + структОшибки.ТипОбъекта + "
		|		Место обнаружения: " + структОшибки.МестоОбнаружения + "
		|		Текст ошибки: " + структОшибки.ТекстОшибки + "
		|		Уточнение: " + структОшибки.Уточнение + "
		|		Путь к объекту: " + структОшибки.Путь;
		
		Лог_Отладка(ТекстСообщения, ВыводитьСообщения);		
	КонецЕсли;
...
Функция АбсолютныйПутьОбъекта(ТекущийОбъект, ТипОбъекта, МестоОбнаружения, ПутьКОбъекту)
	...
	Если СтрокаПоискаСинонима <> Неопределено Тогда
		...
		Иначе
			Лог_Отладка("Неопознаный модуль: " + МестоОбнаружения, ВыводитьСообщения);
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Значение) Тогда
		...
		Лог_Отладка("Не удалось получить путь для: " + ПутьКОбъекту, ВыводитьСообщения);
...
Функция ПолучитьПутьКРодителю(Знач ПутьКОбъекту, Знач ТекущийОбъект)
	...
	Если Не ЗначениеЗаполнено(СинонимXML) Тогда
		...
		ВыводитьСообщения = УровеньЛогирования = 1 Или УровеньЛогирования = 2;
		Лог_Отладка("Синоним не найден: " + ПутьКОбъекту, ВыводитьСообщения);
...
Функция ВыгрузитьОшибкиАПКReportJSON(РезультатыПроверки, МассивГруппировок)
	...
	Лог_Отладка("Для выгрузки: " + КоличествоДляВыгрузки + " ошибок", ВыводитьСообщения);
	...
	Лог_Отладка("Файл с ошибками создан: " + ИмяФайлаРезультата, ВыводитьСообщения);
...
Функция ВыгрузитьОшибкиАПКGenericIssue(РезультатыПроверки, МассивГруппировок)
	...
	КоличествоДляВыгрузки 	= РезультатыПроверки.НайтиСтроки(Новый Структура("Проверено", Истина)).Количество();
	ВыводитьСообщения 		= УровеньЛогирования = 2;
	Лог_Отладка(СтрШаблон("Для выгрузки: %1 ошибок", КоличествоДляВыгрузки), ВыводитьСообщения);
...
Функция ПодготовитьОшибкиАПК(РезультатыПроверки, МассивГруппировок)
	...
	Для Каждого ПутьКОбъекту Из МассивГруппировок Цикл
		...
		Иначе
			Лог_Отладка("Файл не существует: " + Файл.ПолноеИмя, ВыводитьСообщения);
		КонецЕсли;
				...
				Иначе
					...
					Если НачальныйУказатель > 0 И ДлинаСтроки <> 0 Тогда
						...
						Если КонечныйУказатель = 0 Тогда // предположим что это пустая строка
							...
							Лог_Отладка("КонечныйУказатель = 0. Установлена ошибка на весь модуль: " + ПутьКОбъекту, ВыводитьСообщения);
							Лог_Отладка("		Ошибка: " + СтрЗаменить(НайденнаяСтрока.ТекстОшибки, Символы.ПС, Символы.ПС + "		"), ВыводитьСообщения);
							Лог_Отладка("		Содержимое строки: " + СодержимоеСтроки, ВыводитьСообщения);
						...
						Если НачальныйУказатель = КонечныйУказатель Тогда
							Если КонечныйУказатель = 1 Тогда
								...
								Лог_Отладка("НачальныйУказатель = КонечныйУказатель. Установлена ошибка на весь модуль: " + ПутьКОбъекту, ВыводитьСообщения);
								Лог_Отладка("		Ошибка: " + СтрЗаменить(НайденнаяСтрока.ТекстОшибки, Символы.ПС, Символы.ПС + "		"), ВыводитьСообщения);
								Лог_Отладка("		Содержимое строки: " + СодержимоеСтроки, ВыводитьСообщения);
					...
					Иначе
						ПроверкаПройдена = Ложь;
						Лог_Отладка("Проверка не пройдена для: " + ПутьКФайлу, ВыводитьСообщения);
						Лог_Отладка("		Ошибка: " + СтрЗаменить(НайденнаяСтрока.ТекстОшибки, Символы.ПС, Символы.ПС + "		"), ВыводитьСообщения);
					КонецЕсли;
...
Функция ОбработатьОшибкиАПК(ВыборкаОшибокАПК, РезультатыПроверки, МассивГруппировок)
	...
	ВыводитьСообщения     = УровеньЛогирования = 2;
	Лог_Отладка("Ошибок к подготовке: " + КоличествоОшибокОбщее, ВыводитьСообщения);
	...
	Лог_Отладка("Подготовлено ошибок: " + КоличествоОбработано, ВыводитьСообщения);
