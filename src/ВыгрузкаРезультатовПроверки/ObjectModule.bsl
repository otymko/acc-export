...
Функция ПолныйКодОшибки( Знач ВыборкаОшибокАПК )
...
Функция ВыгрузитьОшибкиАПК(РезультатыПроверки, МассивГруппировок)
	...
	Если ФорматЭкспорта = ФорматЭкспортаGenericIssue Тогда
...
Функция ВыгрузитьОшибкиАПКGenericIssue(РезультатыПроверки, МассивГруппировок)
	...
	ИмяФайла = КаталогПроекта + "\" + "acc-generic-issue.json";
	...
	Для Каждого Группировка Из МассивГруппировок Цикл
		...
		Для Каждого СтрокаПроверки Из СтрокиПроверки Цикл
			...
			Запись.ЗаписатьЗначение("src/" + Группировка);
			
			Если СтрокаПроверки.Проверено Тогда
...
Функция ОбработатьОшибкиАПК(ВыборкаОшибокАПК, РезультатыПроверки, МассивГруппировок)
	...
	Пока ВыборкаОшибокАПК.Следующий() Цикл
		...
		Результат.КодОшибки = ПолныйКодОшибки(ВыборкаОшибокАПК);
...
Функция СписокФорматовВыгрузки() Экспорт
	
	Список = Новый СписокЗначений;
	Список.Добавить(ФорматЭкспортаReportJSON);
	Список.Добавить(ФорматЭкспортаGenericIssue);
	Возврат Список;
...
ФорматЭкспортаReportJSON = "ReportJSON";
ФорматЭкспортаGenericIssue = "GenericIssue";
