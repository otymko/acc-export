...
Функция АбсолютныйПутьОбъекта(ТекущийОбъект, ТипОбъекта, МестоОбнаружения, ПутьКОбъекту)
	...
	путь = КэшПутей.Получить( ПутьКОбъекту );
	...
	Значение = "";
	Если СтрокаПоискаСинонима <> Неопределено
		И СтрНайти(МестоОбнаружения, "стр.") Тогда
	...
	Если СтрНайти(МестоОбнаружения, "стр.")
		И Не ЗначениеЗаполнено( Значение ) Тогда
		
		Сообщить( "Не удалось получить путь для : " + ПутьКОбъекту );
		
	КонецЕсли;
	
	КэшПутей.Вставить( ПутьКОбъекту, Значение );
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			...
			Если ПроверкаПройдена Тогда
				...
				Иначе
					...
					Иначе
						ПроверкаПройдена = Ложь;
						Сообщить( "Проверка не пройдена для: " + ПутьКФайлу );
						Сообщить( "		Ошибка: " + СтрЗаменить( НайденнаяСтрока.ТекстОшибки, Символы.ПС, Символы.ПС + "		") );
					КонецЕсли;
